...
struct Squiggle: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        let boundingRectangle = path.boundingRect
        path = path.offsetBy(dx: rect.minX - boundingRectangle.minX, dy: rect.minY - boundingRectangle.minY)

        let scale: CGFloat = rect.width / boundingRectangle.width
